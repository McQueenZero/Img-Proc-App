...
class Ui_SubWindow(QtWidgets.QWidget):
    ...
    def retranslateSubUi(self, SubWindow):
        ...
        self.pushButton_Apply.setShortcut(_translate("MainWindow", "Enter"))
        ...
        self.pushButton_Cancel.setShortcut(_translate("MainWindow", "Ctrl+S"))
    ...
    def reset(self):
        ...
        k_reciprocal = 1 / self.graphicsView.k_rec  # 滚轮缩放系数记录值的倒数
        ...
        self.graphicsView.imItem.setPos(0, 0)  # 重设场景矩形范围并设置位置
    ...
    def filter(self):
        ...
        if self.pushButton_1.isChecked():
            ...
            w_dict = [0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 0.96, 0.98, 1, 1.03, 1.05]
            self.horizontalSlider.setEnabled(True)
            self.horizontalSlider.setValue(5)
            self.label_Num.setNum(5)
            self.horizontalSlider.valueChanged['int'].connect(self.label_Num.setNum)
            self.horizontalSlider.valueChanged.connect(
                lambda: self.defog(w_dict[self.horizontalSlider.value()]))
            self.defog()
            ...
            rr_dict = [(1, 1), (2, 1), (2, 2), (3, 2), (3, 3), (4, 3),
                       (5, 3), (5, 4), (6, 4), (6, 5), (7, 5)]
            self.horizontalSlider.setEnabled(True)
            self.horizontalSlider.setValue(5)
            self.label_Num.setNum(5)
            self.horizontalSlider.valueChanged['int'].connect(self.label_Num.setNum)
            self.horizontalSlider.valueChanged.connect(
                lambda: self.coloreq(rr_dict[self.horizontalSlider.value()]))
            self.coloreq()
            ...
            bC_dict = [(5, 15), (7, 15), (9, 10), (11, 10), (13, 5), (15, 5),
            ...
            self.horizontalSlider.valueChanged['int'].connect(self.label_Num.setNum)
            self.horizontalSlider.valueChanged.connect(
                lambda: self.enhshp(bC_dict[self.horizontalSlider.value()]))
            self.enhshp()
            ...
            self.pushButton_Apply.setEnabled(False)
            self.disp(DSTFLAG=2)
            self.graphicsView.scale(1 / self.graphicsView.k_rec, 1 / self.graphicsView.k_rec)
            self.graphicsView.k_rec = 1
            self.graphicsView.scene.setSceneRect(self.graphicsView.imItem.boundingRect())
    ...
    def more(self):
        ...
        if self.pushButton_1.isChecked():
            ...
            self.fontset(QtGui.QFont('黑体', 32))
            ...
            self.colorset(QtGui.QColor('white'))
    ...
    def colorother(self):
        ...
        if self.graphicsView.imItem.drawMode != 5:
            color = QtWidgets.QColorDialog.getColor(
                QtGui.QColor('black'), self, '更多颜色', QtWidgets.QColorDialog.ShowAlphaChannel)
            self.colorset(color)
        else:
            color = QtWidgets.QColorDialog.getColor(QtGui.QColor('black'), self, '更多颜色')
            self.colorset(color)
    ...
    def fontother(self):
        ...
        打开字体对话框选择字体
        '''
        font, confirm = QtWidgets.QFontDialog.getFont(QtGui.QFont('黑体', 32), self, '更多字体')
        if confirm:
            self.fontset(font)
    ...
    def transform(self):
        ...
        透视变换槽函数
        '''
        s_old = self.graphicsView.imItem.pixmap().size()
        ...
        s_new = self.graphicsView.imItem.pixmap().size()
        self.scalePixView(s_new, s_old)
    ...
    def apply(self):
        ...
        '''
        if self.ModeFlag == 3:
            if self.graphicsView.imItem.drawMode == 5 or self.graphicsView.imItem.drawMode == 6:
    ...
    def disp(self, DSTFLAG=1, CACHEFLAG=True, GRAYFLAG=False, PNGFLAG=False):
        '''
        显示刷新方法，接收缓存判断、CVdst/src判断和灰度转换选项输入
        完成将CV类图片转换成QImage并在ImgView上显示
        '''
        if DSTFLAG == 1:
    ...
    def scalePixView(self, s_new: QtCore.QSize, s_old: QtCore.QSize):
        '''
        根据disp前后pixmap大小确定显示大小比例
        从而进行自适应放缩并居中
        '''
        k_w, k_h = s_new.width() / s_old.width(), s_new.height() / s_old.height()
